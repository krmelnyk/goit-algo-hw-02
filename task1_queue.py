...
def generate_request():
    '''Genetate a new request and add it to the queue'''
    global request_id
    request_id += 1
    ...
    print(f"Generated request {request_id} with priority {request['priority']}")

def process_request():
    '''Process a request from the queue'''
    if not request_queue.empty():
        request = request_queue.get()
        print(f"Processing request {request['id']} with priority {request['priority']}")
